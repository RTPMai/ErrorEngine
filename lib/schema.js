...
export const FIELDS = {
  ...
  invoice_ref:   { type: "string", required: false, source: "printavo",      def: "Printavo invoice / visual ID the error belongs to." },
  customer_id:   { type: "string", required: false, source: "backbone",      def: "BackBone customer_id — the join key into backbone_data." },
  ...
  description:   { type: "string", required: false, source: "user",          def: "What happened and why." },
  units:         { type: "number", required: false, source: "user",          def: "Units affected." },
  cost:          { type: "number", required: false, source: "computed/user", def: "Dollar cost (reprint qty x price chart)." },
